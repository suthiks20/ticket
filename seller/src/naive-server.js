// seller/src/naive-server.js (DELIBERATELY BROKEN)
...
fastify.post('/reset', async (request, reply) => {
  ...
  if (!Number.isInteger(ticket_count) || ticket_count < 1) return reply.code(400).send({ error: 'Invalid' });
...
async function start() {
  try {
    await pool.query('CREATE TABLE IF NOT EXISTS sales (sale_id UUID PRIMARY KEY DEFAULT gen_random_uuid(), ticket_count INT, is_active BOOLEAN); CREATE TABLE IF NOT EXISTS tickets (sale_id UUID, ticket_number INT, user_id TEXT, request_id TEXT);');
